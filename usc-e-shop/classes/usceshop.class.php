...
<?php
	...
	function google_recaptcha_v3_response( $token, $secret ) {
		...
		if ( ! $response_body['success'] || ! in_array( $action, array( 'create_new_member', 'customer_order_page' ) ) ) {
			return $is_human;
		}
	...
	function get_itemByCode( $item_code ) {
		...
		$cache_key = 'wel_post_by_code_' . $item_code;
		...
		if ( false === $post_data ) {
			...
			$query = $wpdb->prepare( "SELECT * FROM $wpdb->posts AS `post` 
				...
				'_itemCode', $item_code, 'trash', 'inherit', 'auto-draft' );
			$post_data = $wpdb->get_results( $query );
			if ( null !== $post_data ) {
				wp_cache_set( $cache_key, $post_data );
			}
	...
	function get_skus( $post_id, $keyflag = 'sort' ) {
		...
		foreach( $metas as $rows ){
			...
			if ( ! isset( $values['advance'] ) ) {
				$values['advance'] = '';
			}
			$values['meta_id'] = $rows['meta_id'];
			$skus[$key] = $values;
		}
