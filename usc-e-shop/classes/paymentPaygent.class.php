...
<div id="settlement_dialog" title="">
				...
				if ( 0 !== (int) $acting_opts['payment_class'] ) {
					...
					if ( isset( $latest_log['log']['split_count'] ) && isset( $latest_log['log']['payment_class'] ) ) {
						switch ( $latest_log['log']['payment_class'] ) {
							case '10':
								$split_count = ' (' . __( 'One time payment', 'usces' ) . ')';
								break;
							case '23':
								$split_count = ' (' . __( 'Bonus lump-sum payment', 'usces' ) . ')';
								break;
							case '80':
								$split_count = ' (' . __( 'Libor Funding pay', 'usces' ) . ')';
								break;
							default:
								$times = (int) $latest_log['log']['split_count'];
								if ( 0 < $times ) {
									$split_count = ' (' . $times . __( '-time payment', 'usces' ) . ')';
								}
						}
	...
	public function payment_detail( $str, $entry ) {
		...
		if ( isset( $payment['settlement'] ) && 'acting_paygent_card' == $payment['settlement'] ) {
			$acting_opts = $this->get_acting_settings();
			if ( 0 !== (int) $acting_opts['payment_class'] ) {
				$split_count = ( isset( $_POST['split_count'] ) ) ? $_POST['split_count'] : '01';
				switch ( $split_count ) {
					case '01':
				...
				if ( usces_is_login() && defined( 'WCEX_MSA' ) && isset( $entry['delivery']['delivery_flag'] ) && 2 == $entry['delivery']['delivery_flag'] ) {
					...
					if ( 0 < $count_msa ) {
						...
						foreach ( $msacart as $group_id => $group ) {
							...
							if ( 0 == $idx_msa ) {
								$line1 = trim( $destination_info['msa_address2'] );
								$line2 = trim( $destination_info['msa_address3'] );
								...
								$additional_shipping_addresses .= "\t\t\t\t" . '"line1": "' . trim( $destination_info['msa_address2'] ) . '",' . "\n";
								$additional_shipping_addresses .= "\t\t\t\t" . '"line2": "' . trim( $destination_info['msa_address3'] ) . '",' . "\n";
					...
					$line1 = trim( $entry['delivery']['address2'] );
					$line2 = trim( $entry['delivery']['address3'] );
	...
	public function update_settlement( $form, $member ) {
		...
		if ( isset( $acting_opts['card_activate'] ) && 'off' != $acting_opts['card_activate'] && 'on' == $acting_opts['stock_card_mode'] && ! empty( $member['ID'] ) ) {
			...
			if ( isset( $customer['customer_id'] ) && $customer['customer_id'] == $member['ID'] ) {
				...
				$form                 .= '<div class="gotoedit"><a href="' . $update_settlement_url . '">' . __( 'Change the credit card is here >>', 'usces' ) . '</a></div>';
			} else {
				...
				$form                   .= '<div class="gotoedit"><a href="' . $register_settlement_url . '">' . __( 'Credit card registration is here >>', 'usces' ) . '</a></div>';
			}
		}
		return $form;
	}
...
<div class="whitebox">
	...
	<form id="member-card-info" action="<?php echo esc_url( $update_settlement_url ); ?>" method="post" onKeyDown="if(event.keyCode == 13) {return false;}">
			<?php echo $form; ?>
		<div class="send">
			...
			<input type="button" id="card-delete" class="card-delete" data-update_mode="delete" value="<?php esc_attr_e( 'Delete', 'usces' ); ?>" />
				<?php endif; ?>
			<?php endif; ?>
			<input type="button" name="back" value="<?php _e( 'Back to the member page.', 'usces' ); ?>" onclick="location.href='<?php echo esc_url( USCES_MEMBER_URL ); ?>'" />
