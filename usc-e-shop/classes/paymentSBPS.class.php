	...
	public function update_settlement( $html, $member ) {
		...
		if ( 'on' == $acting_opts['cust_manage'] || 'choice' == $acting_opts['cust_manage'] ) {
			...
			if ( isset( $cust_ref['result'] ) && 'OK' === $cust_ref['result'] ) {
				...
				$html .= '<div class="gotoedit"><a href="' . $update_settlement_url . '">' . __( "Change the credit card is here >>", 'usces' ) . '</a></div>';
			} else {
				$register_settlement_url = add_query_arg( array( 'usces_page' => 'member_register_settlement', 're-enter' => 1 ), USCES_MEMBER_URL );
				$html .= '<div class="gotoedit"><a href="' . $register_settlement_url . '">' . __( "Credit card registration is here >>", 'usces' ) . '</a></div>';
			}
		}
		return $html;
	}
