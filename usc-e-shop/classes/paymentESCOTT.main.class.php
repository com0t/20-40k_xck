	...
	public function order_confirm_mail_payment( $msg_payment, $order_id, $payment, $cart, $data ) {
		...
		if( $this->acting_flg_card == $payment['settlement'] ) {
			...
			if( 1 === (int)$acting_opts['howtopay'] ) {
				...
				if( isset( $acting_data['PayType'] ) ) {
					...
					switch ( $acting_data['PayType'] ) {
						case '01':
							$paytype = ' ('.__( 'One time payment', 'usces' ).')';
							break;
						case '02':
						case '03':
						case '05':
						...
						case '24':
							...
							$paytype = ' ('.$times.__( '-time payment', 'usces' ).')';
							break;
						case '80':
							$paytype = ' ('.__( 'Bonus lump-sum payment', 'usces' ).')';
							break;
						case '88':
							$paytype = ' ('.__( 'Libor Funding pay', 'usces' ).')';
							break;
					}
					if ( ! empty( $paytype ) ) {
			...
			if ( usces_is_html_mail() ) {
				...
				$msg_payment .= sprintf( __( "Payment expiration date is %s days.", 'usces' ), $acting_opts['conv_limit'] ) . '<br>';
				$msg_payment .= __( "If payment has not yet been completed, please payment procedure from the following URL.", 'usces' ) . '<br><br>';
				// $msg_payment .= __( "[Payment URL]", 'usces' ) . '<br>';
				$msg_payment .= '<a href="' . $url . '">' . $url . '</a><br>';
				$msg_payment .= '</td></tr>';
			} else {
		...
		if ( isset( $fields['acting'] ) && isset( $fields['MerchantFree1'] ) ) {
			switch ( $fields['acting'] ) {
				...
				case $this->acting_card:
					$keys = array( 'acting', 'TransactionId', 'MerchantFree1', 'ResponseCd', 'PayType', 'KessaiNumber', 'NyukinDate', 'SecureResultCode' );
					break;
				/* コンビニ決済 */
...
<?php
	...
	public function order_mail_payment( $msg_payment, $order_id, $payment, $cart, $entry, $data ) {
		...
		$url = $usces->get_order_meta_value( $this->paymod_id.'_conv_url', $order_id );
		if ( usces_is_html_mail() ) {
			$msg_payment  = '<tr><td colspan="2" style="padding: 0 0 25px 0;">' . $payment['name'] . usces_payment_detail( $entry ) . '<br><br>';
			$msg_payment .= sprintf( __( "Payment expiration date is %s days.", 'usces' ), $acting_opts['conv_limit'] ) . '<br>';
			$msg_payment .= __( "If payment has not yet been completed, please payment procedure from the following URL.", 'usces' ) . '<br><br>';
			// $msg_payment .= __( "[Payment URL]", 'usces' ) . '<br>';
			$msg_payment .= '<a href="' . $url . '">' . $url . '</a><br>';
