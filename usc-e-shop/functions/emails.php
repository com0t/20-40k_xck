...
function usces_send_ordermail( $order_id ) {
	...
	if ( usces_is_html_mail() ) {
		...
		// add body
		$message .= '<tr><td style="padding:20px 30px;">';
		$message .= apply_filters( 'usces_filter_send_order_mail_bodyall', $msg_body, $data );
		$message .= '</td></tr>';
		// add footer
		$message .= '<tr><td style="padding:20px 30px;">';
		...
		if ( 1 === (int) $usces->options['put_customer_name'] ) {
			// translators: %s: name of user.
			$dear_name = sprintf( __( 'Dear %s', 'usces' ), usces_localized_name( trim( $data['order_name1'] ), trim( $data['order_name2'] ), 'return' ) );
			if ( false !== strpos( $header, '{customer_name}' ) ) {
				$m_header = str_replace( '{customer_name}', $dear_name, $m_header );
			} else {
				$message .= $dear_name . "\r\n\r\n";
			}
 ...
 *
 * @param array $args : compact( 'order_id', 'cart', 'newcart', 'entry', 'payment', 'data' )
 *
...
function usces_get_thanksmail_htmlbody( $args ) {
	...
	foreach ( $cart as $cart_key => $cart_row ) {
		...
		$meisai  .= apply_filters( 'usces_filter_cart_item_name_nl', $cartItemName, $args ) . '<br>';
		...
		$meisai .= '</td>';
		$meisai .= '<td style="text-align: center; width: 25%; padding: 12px; border: 1px solid #ddd; vertical-align: middle;">' . usces_crform( $skuPrice, true, false, 'return' ) . '</td>';
	...
	if ( usces_is_tax_display() && 'products' === usces_get_tax_target() ) {
		...
		$meisai .= '<td style="text-align: right; width: 75%; padding: 12px; border: 1px solid #ddd; vertical-align: middle;">' . ( ( 'exclude' === usces_get_tax_mode() ) ? usces_tax( $entry, 'return' ) : 0 ) . '</td>';
		$meisai .= '</tr>';
	}
 ...
 *
 * @param array $args : compact( 'order_id', 'data', 'deli', 'cart', 'country', 'customer', 'condition', 'total_full_price', 'mail_data', 'payment', 'tax_display', 'reduced_taxrate', 'tax_mode', 'tax_target', 'usces_tax', 'point_coverage', 'member_system', 'member_system_point' )
 *
 * @return text : mail body
 */
function usces_get_adminmail_htmlbody( $args ) {
	...
	if ( 'mitumoriConfirmMail' == $_POST['mode'] ) {
		$msg_body .= '<table style="font-size: 14px; margin-bottom: 30px; width: 100%; border-collapse: collapse; border: 1px solid #ddd;">';
	...
	if ( 'completionMail' == $_POST['mode'] ) {
		...
		if ( ! empty( $delivery_company ) ) {
			...
			if ( ! empty( $tracking_number ) ) {
				...
				$msg_shipping .= '<td style="padding: 0 0 10px 50px; width: calc( 100% - 100px );">' . $tracking_number . '<p>' . __( "[*]", 'usces' ) . __( 'You can check delivery situation from the following URL.', 'usces' ) . '</p>';
				$msg_shipping .= '<p>' . usces_get_delivery_company_url( $delivery_company, $tracking_number ) . '</p>' . '</td>';
				$msg_shipping .= '</tr>';
			}
		}
	}
...
function uesces_get_mail_addressform( $type, $data, $order_id, $out = 'return' ){
	...
	foreach ($valueDefault as $key => $val) {
		if (!isset($values[$key])) {
			$values[$key] = $valueDefault[$key];
		}
	}
