	...
	if ( empty( $skus ) ) {
		...
		<table id="skulist-table" class="list">
			...
			<tr>
				...
				<th class="item-sku-zaiko"><?php _e('stock status','usces'); ?></th><?php echo apply_filters( 'usces_filter_sku_meta_title1', '' ); ?>
			</tr>
			<tr>
...
function _list_item_sku_meta_row( $sku ) {
	...
	ob_start();
	?>
	<tr class='metastuffrow'><td colspan='<?php echo $sku_colspan; ?>'>
...
function del_item_sku_meta( $post_ID ) {
	...
	$post_ID = (int) $post_ID;
	$skumetaid = isset($_POST['skumetaid']) ? (int)$_POST['skumetaid'] : '';
	$res = apply_filters( 'usces_filter_before_del_item_sku_meta', false, $post_ID, $skumetaid );
	if( false !== $res ){
		return $res;
	}
