	...
	if( !empty( $results ) ) {
		...
		foreach( $lines as $rows_num => $line ) {
			...
			if( $pre_code != $item_code ) {
				...
				$cfdata = array();
				$cfrows = ( 0 === $usces->options['system']['csv_encode_type'] ) ? explode( USCES_CF_DELIMITER, trim( mb_convert_encoding( $datas[ USCES_COL_CUSTOM_FIELD ], 'UTF-8', 'SJIS' ) ) ) : explode( USCES_CF_DELIMITER, trim( $datas[ USCES_COL_CUSTOM_FIELD ] ) );
				...
				foreach( $item_custom_fields as $key ) {
					...
					if( is_array($values) ) {
						foreach( $values as $value ) {
							$icfield .= usces_entity_decode( $key, $ext ).$eq.usces_entity_decode( $value, $ext ).$cf_sp;
						}
					} else {
						$icfield .= usces_entity_decode( $key, $ext ).$eq.usces_entity_decode( $values, $ext ).$cf_sp;
					}
				}
			}
			$cfield = '';
...
function usces_item_stock_uploadcsv( $lines ) {
	...
	foreach( $lines as $rows_num => $line ) {
		...
		$sku_meta = usces_get_post_meta( $post_id, '_isku_', false );
		if( !$sku_meta ) {
			$err_num++;
			$mes = "No.".$line_num."\t".__('This data was not registered in the database.', 'usces');
			$log .= $mes.$yn;
