...
usces_admin_mail = {
	...
	filterContentbeforePreview: function( elEmail ) {
		...
		if (tinymce.get(elEmailHeader)) {
			contentEmailHeader = tinymce.get(elEmailHeader).getContent();
		} else {
			var headerText = jQuery("#"+elEmailHeader).text();
			contentEmailHeader = headerText.replace(/(?:\r\n|\r|\n)/g, '<br>');
		}
		if (tinymce.get(elEmailfooter)) {
			contentEmailFooter = tinymce.get(elEmailfooter).getContent();
		} else {
			var footerText = jQuery("#"+elEmailfooter).text();
			contentEmailFooter = footerText.replace(/(?:\r\n|\r|\n)/g, '<br>');
		}
		...
		s.data = {
			...
			'content_header': contentEmailHeader,
			'content_footer': contentEmailFooter,
		};
