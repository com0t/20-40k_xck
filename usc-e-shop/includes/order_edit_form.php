...
if ( 'new' === $order_action ) {
	...
	if ( isset( $exopt['system']['datalistup']['orderlist_flag'] ) && $exopt['system']['datalistup']['orderlist_flag'] ) {
		$prev_uri   = esc_url_raw( $this->get_prev_page_uri( 'order', $order_id ) );
		$next_uri   = esc_url_raw( $this->get_next_page_uri( 'order', $order_id ) );
		$navibutton = '';
		if ( ! empty( $prev_uri ) ) {
		...
		<td colspan="2" rowspan="2" class="wrap_td">
			...
			<table border="0" cellspacing="0" class="cus_info">
				...
				<tr>
					...
					?>
					<span><?php esc_html_e( 'Special Benefits', 'usces' ); ?> : </span><?php echo $condition["campaign_privilege"]; ?> (<?php if ( $condition["campaign_privilege"] == 'discount') { esc_html_e( $condition["privilege_discount"] ) . __('% Discount', 'usces');} elseif ( $condition["campaign_privilege"] == 'point' ){echo esc_html( $condition["privilege_point"] ) . __(" times (limited to members)", 'usces');} ?>) <br />
